...
export function FunctionRunner({
  ...
  const run = async () => {
    ...
    try {
      ...
      if (isView) {
        ...
        if (!signer) {
          setResult("Write actions require a signer (connect wallet).");
          return;
        }
  ...
  const filtered = useMemo(() => {
    ...
    return fragments.filter((f) => prettySig(f.format()).toLowerCase().includes(q));
  }, [fragments, query]);
    ...
    <div className="space-y-5">
      {fragments.length === 0 && (
        <div className="rounded-xl border border-rose-500/25 bg-rose-500/10 px-4 py-3 text-sm text-rose-100">
          ABI looks empty or invalid. Check <code>src/config/contract.js</code>.
        </div>
      )}
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="w-full lg:w-[70%] min-w-0" ref={dropdownRef}>
          ...
          <div className="neo-help mt-1">
            Direct contract invocation — useful for debugging and edge cases.
          </div>
            ...
            {open && (
              <div className="absolute left-0 top-full mt-2 w-full z-50 rounded-xl overflow-hidden neo-dropdown">
                ...
                <div className="p-2 border-b border-white/10">
                  <input
                    className="neo-input"
                    placeholder="Search methods… (e.g., verifyCredential)"
                    value={query}
        ...
        <div className="neo-badge mt-7 lg:mt-0">
          <span
            className={`inline-block h-2.5 w-2.5 rounded-full ${
              isView ? "bg-emerald-300/80" : "bg-amber-300/80"
            }`}
          />
        ...
        <div className="rounded-2xl border border-white/10 bg-black/20 px-6 py-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`neo-badge ${
                  isView ? "" : "border-amber-500/25 bg-amber-500/10 text-amber-100"
                }`}
              >
          ...
          <div className="mt-6">
            ...
            <div className="neo-help mt-1">
              Provide inputs in the exact ABI types shown (prevents bad formatting and failed calls).
            </div>
              ...
              {inputs.map((inp, i) => (
                <div key={i} className="min-w-0">
                  <div className="flex items-center justify-between gap-3">
                    <div className="neo-label">{inp.name || `arg${i}`}</div>
                    <div className="text-xs text-zinc-500">{inp.type}</div>
                  </div>
                  <input
                    ...
                    value={args[i] ?? ""}
                    onChange={(e) =>
                      setArgs((a) => a.map((v, idx) => (idx === i ? e.target.value : v)))
                    }
                  />
                  <div className="neo-help mt-2">{helpFor(inp.type)}</div>
                </div>
              ))}
            </div>
...
function prettySig(sig) {
  ...
  try {
    ...
    const inside = sig.slice(open + 1, sig.lastIndexOf(")")).replace(/,\s*/g, ", ").trim();
    return `${name} (${inside})`;
  } catch {
    return sig;
  }
...
function helpFor(type) {
  ...
  if (type === "bytes32") return "Paste a 0x... 32-byte hash (hex).";
  if (type.startsWith("uint") || type.startsWith("int")) return "Numbers only (no commas).";
