...
try {
  ...
  const { response: healthResponse, payload: healthPayload } = await request("/health", { useCookie: false });
  ...
  const { response: inviteResendResponse, payload: resentInvitation } = await request(
    `/api/team/invitations/${createdInvitation.id}/resend`,
    {
      method: "POST",
    }
  );
  assert.equal(inviteResendResponse.status, 200);
  assert.equal(resentInvitation.id, createdInvitation.id);
  assert.ok(resentInvitation.lastSentAt);

  const { payload: revokeCandidate } = await request("/api/team/invitations", {
    method: "POST",
    body: JSON.stringify({
      name: "Alex Revoked",
  ...
  const { response: revokeInviteResponse, payload: revokedInvitation } = await request(
    `/api/team/invitations/${revokeCandidate.id}/revoke`,
    {
      method: "PATCH",
    }
  );
  assert.equal(revokeInviteResponse.status, 200);
  ...
  assert.ok(activityBootstrap.activity.some((event) => event.type === "invitation.revoked"));
  ...
  const { response: passwordResetRequestResponse, payload: passwordResetRequestPayload } = await request(
  ...
  assert.match(passwordResetRequestPayload.previewResetUrl, /\/reset-password\//);
  const resetToken = passwordResetRequestPayload.previewResetUrl.split("/reset-password/")[1];

  const { response: passwordResetResponse, payload: passwordResetPayload } = await request(
    "/api/auth/password-reset/reset",
    {
      method: "POST",
      body: JSON.stringify({
        token: decodeURIComponent(resetToken),
        password: "workspace456",
      }),
      useCookie: false,
    }
  );
  assert.equal(passwordResetResponse.status, 200);
  assert.equal(passwordResetPayload.ok, true);

  const { response: invalidatedBootstrapResponse } = await request("/api/bootstrap");
  assert.equal(invalidatedBootstrapResponse.status, 401);

  const { response: postResetLoginResponse, payload: postResetLoginSession } = await request("/api/auth/login", {
    method: "POST",
    body: JSON.stringify({
      workEmail: "jane@acme.example",
      password: "workspace456",
  ...
  assert.equal(postResetLoginResponse.status, 200);
  assert.equal(postResetLoginSession.user.email, "jane@acme.example");
  assert.ok(cookieJar);
