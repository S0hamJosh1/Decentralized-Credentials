...
function normalizeIssuer(issuer = {}) {
  return {
    ...
    walletVerifiedAt: normalizeText(issuer.walletVerifiedAt),
  };
}
...
function normalizeCredential(credential = {}) {
  ...
  return {
    ...
    credentialHash: normalizeText(credential.credentialHash),
    anchorStatus: normalizeText(credential.anchorStatus, "AwaitingIssuerWallet") || "AwaitingIssuerWallet",
    chainId: normalizeText(credential.chainId),
    network: normalizeText(credential.network),
    contractAddress: normalizeText(credential.contractAddress),
    txHash: normalizeText(credential.txHash),
    blockNumber: normalizeText(credential.blockNumber),
    anchoredAt: normalizeText(credential.anchoredAt),
    revokeTxHash: normalizeText(credential.revokeTxHash),
    revokeBlockNumber: normalizeText(credential.revokeBlockNumber),
    revokedOnChainAt: normalizeText(credential.revokedOnChainAt),
    ...
    issuerWallet: normalizeText(credential.issuerWallet),
...
async function readPrismaDb() {
  ...
  return normalizeDb({
    ...
    issuers: issuers.map((issuer) => ({
      ...
      walletVerifiedAt: fromDate(issuer.walletVerifiedAt),
    ...
    if (normalized.issuers.length > 0) {
      await tx.issuer.createMany({
        data: normalized.issuers.map((issuer) => ({
          ...
          walletVerifiedAt: toDate(issuer.walletVerifiedAt),
