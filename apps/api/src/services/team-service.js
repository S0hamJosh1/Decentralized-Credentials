...
function requireOwnerControlForOwnerInvitation(auth, invitation) {
  if (invitation.membershipRole === "Owner") {
    requireOwner(auth, "Only workspace owners can manage owner invitations.");
  }
}
...
function findInvitationForOrganization(db, organizationId, invitationId) {
...
async function deliverInvitationEmail(db, organization, invitation) {
  const invitationRecord = buildInvitationRecord(db, organization, invitation);
  const inviterName = invitationRecord.invitedBy || organization.name;

  return sendWorkspaceEmail({
    to: invitation.email,
    subject: `Join ${organization.name} on Credential Foundry`,
    text: `${invitation.name || invitation.email},\n\n${inviterName} invited you to join ${organization.name} on Credential Foundry.\n\nUse this link to join the workspace:\n${invitationRecord.joinUrl}\n`,
    html: `<p>${invitation.name || invitation.email},</p><p>${inviterName} invited you to join <strong>${organization.name}</strong> on Credential Foundry.</p><p><a href="${invitationRecord.joinUrl}">Join the workspace</a></p><p>${invitationRecord.joinUrl}</p>`,
  });
}
...
export async function createInvitation(auth, payload) {
  ...
  await deliverInvitationEmail(savedDb, savedOrganization, invitation);
...
export async function resendInvitation(auth, invitationId) {
  requireTeamManager(auth);

  const db = await readDb();
  const organization = findOrganization(db, auth.organization.id);
  const invitation = findInvitationForOrganization(db, organization.id, invitationId);

  if (invitation.status !== "Pending") {
    throw createHttpError(400, "Only pending invitations can be resent.");
  }

  requireOwnerControlForOwnerInvitation(auth, invitation);

  invitation.lastSentAt = nowIso();
  recordWorkspaceEvent(db, {
    organizationId: organization.id,
    actorUserId: auth.user.id,
...
export async function revokeInvitation(auth, invitationId) {
  ...
  if (invitation.status !== "Pending") {
    throw createHttpError(400, "Only pending invitations can be revoked.");
  }
