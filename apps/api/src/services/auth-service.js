...
import { sendWorkspaceEmail } from "../lib/email.js";
import {
  ensureUnique,
  requireEmailAddress,
  requireFields,
...
function sanitizePasswordResetRequestPayload(payload = {}) {
  return {
    workEmail: sanitizeEmail(payload.workEmail),
  };
}
...
function removeExpiredPasswordResetTokens(db) {
  const now = Date.now();
...
function createPasswordResetUrl(db, user, token) {
  const membership = listUserMemberships(db, user.id)[0] || null;
  const organization = membership ? findOrganization(db, membership.organizationId) : null;
  const baseUrl =
    sanitizeText(process.env.PUBLIC_APP_URL)
    || sanitizeText(organization?.verificationDomain)
    || "http://localhost:5173";

  return new URL(`/reset-password/${encodeURIComponent(token)}`, baseUrl).toString();
}

function buildPasswordResetResponse(previewResetUrl = "") {
  return {
    ok: true,
...
export async function registerAccount(payload) {
  ...
  removeExpiredPasswordResetTokens(db);
...
export async function requestPasswordReset(payload) {
  ...
  const input = sanitizePasswordResetRequestPayload(payload);
  requireFields(input, ["workEmail"], "password reset request fields");
  input.workEmail = requireEmailAddress(input.workEmail, "work email");
  ...
  db.passwordResetTokens.unshift({
    id: nextId("PRT-", db.passwordResetTokens, 1000),
    userId: user.id,
  ...
  const resetUrl = createPasswordResetUrl(db, user, rawToken);
  const emailResult = await sendWorkspaceEmail({
    to: user.email,
    subject: "Reset your Credential Foundry password",
    text: `Use this link to reset your workspace password: ${resetUrl}\n\nThis link expires in one hour.`,
    html: `<p>Use this link to reset your workspace password:</p><p><a href="${resetUrl}">${resetUrl}</a></p><p>This link expires in one hour.</p>`,
  });
  ...
  return buildPasswordResetResponse(previewResetUrl);
}
...
export async function resetPassword(payload) {
  ...
  const input = sanitizePasswordResetPayload(payload);
  requireFields(input, ["token", "password"], "password reset fields");
  ...
  const resetToken = db.passwordResetTokens.find((token) => token.tokenHash === tokenHash);

  if (!resetToken) {
    throw createHttpError(400, "That password reset link is invalid or has expired.");
  }

  if (resetToken.usedAt || Date.parse(resetToken.expiresAt) <= Date.now()) {
    throw createHttpError(400, "That password reset link is invalid or has expired.");
  }
  ...
  user.authProvider = mergeAuthProvider(user.authProvider, "password");
